...
class Dao {
  ...
  addNewMessage(message) {
    return this.db.executeQuery('run', `INSERT OR IGNORE INTO messages
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)`,
    [message.nodeId, message.id, message.messageId, message.text, message.dateTime, message.sender, message.chatId])
  }
  ...
  getLastMessageIds() {
    return this.db.executeQuery('all', `SELECT node_id, MAX(id) 
      FROM messages WHERE node_id IS NOT NULL AND id IS NOT NULL GROUP BY node_id`)
  }
